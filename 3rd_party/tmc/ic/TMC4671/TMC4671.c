...
// => SPI wrapper
//extern uint8_t tmc4671_readwriteByte(uint8_t data, uint8_t isAddress, uint8_t isReadOp, uint8_t lastTransfer);
// <= SPI wrapper

// spi access
extern int32_t tmc4671_readInt(uint8_t address);
//{
//	// clear write bit
//	address &= 0x7F;
//
//	// write address
//	tmc4671_readwriteByte(address, true, true, false);
//
//	// read data
//	int32_t value = tmc4671_readwriteByte(0, false, true, false);
//	value <<= 8;
//	value |= tmc4671_readwriteByte(0, false, true, false);
//	value <<= 8;
//	value |= tmc4671_readwriteByte(0, false, true, false);
//	value <<= 8;
//	value |= tmc4671_readwriteByte(0, false, true, true);
//
//	return value;
//}

extern void tmc4671_writeInt(uint8_t address, int32_t value);
//{
//	// write address
//	tmc4671_readwriteByte(address|0x80, true, false, false);
//
//	// write value
//	tmc4671_readwriteByte(0xFF & (value>>24), false, false, false);
//	tmc4671_readwriteByte(0xFF & (value>>16), false, false, false);
//	tmc4671_readwriteByte(0xFF & (value>>8), false, false, false);
//	tmc4671_readwriteByte(0xFF & (value>>0), false, false, true);
//}
...
int32_t tmc4671_getActualRampTorque_raw()
{
	// no ramp implemented
	return 0;
}
...
int32_t tmc4671_getActualRampVelocity()
{
	// no ramp implemented
	return 0;
}
...
int32_t tmc4671_getActualRampPosition()
{
	// no ramp implemented
	return 0;
}
		...
		uint16_t *last_Phi_E_Selection, uint32_t *last_UQ_UD_EXT, int16_t *last_PHI_E_EXT)
{
	...
	case STATE_START_INIT: // started by writing 1 to initState
		...
		*last_PHI_E_EXT = (int16_t) tmc4671_readRegister16BitValue(TMC4671_PHI_E_EXT, BIT_0_TO_15);
		...
		tmc4671_writeInt(TMC4671_MODE_RAMP_MODE_MOTION, TMC4671_MOTION_MODE_UQ_UD_EXT);
		...
		int16_t *hall_phi_e_old, int16_t *hall_phi_e_new, int16_t *hall_actual_coarse_offset, uint16_t *last_Phi_E_Selection)
{
	...
	case STATE_START_INIT: // started by writing 1 to initState
		...
		*hall_phi_e_old = TMC4671_FIELD_READ(TMC4671_HALL_PHI_E_INTERPOLATED_PHI_E, TMC4671_HALL_PHI_E_MASK, TMC4671_HALL_PHI_E_SHIFT);

		// read actual abn_decoder angle and compute difference to actual hall angle
		*hall_actual_coarse_offset = tmc4671_getS16CircleDifference(*hall_phi_e_old, (int16_t) tmc4671_readRegister16BitValue(TMC4671_ABN_DECODER_PHI_E_PHI_M, BIT_16_TO_31));

		// set ABN_DECODER_PHI_E_OFFSET to actual hall-abn-difference, to use the actual hall angle for coarse initialization
		tmc4671_writeRegister16BitValue(TMC4671_ABN_DECODER_PHI_E_PHI_M_OFFSET, BIT_16_TO_31, *hall_actual_coarse_offset);

		// normally MOTION_MODE_UQ_UD_EXT is only used by e.g. a wizard, not in normal operation
		if (TMC4671_FIELD_READ(TMC4671_MODE_RAMP_MODE_MOTION, TMC4671_MODE_MOTION_MASK, TMC4671_MODE_MOTION_SHIFT) != TMC4671_MOTION_MODE_UQ_UD_EXT)
		{
			// select the use of phi_e_hall to start motor with hall signals
			tmc4671_writeRegister16BitValue(TMC4671_PHI_E_SELECTION, BIT_0_TO_15, TMC4671_PHI_E_HALL);
		}
	...
	case STATE_WAIT_INIT_TIME:
		// read actual hall angle
		*hall_phi_e_new = TMC4671_FIELD_READ(TMC4671_HALL_PHI_E_INTERPOLATED_PHI_E, TMC4671_HALL_PHI_E_MASK, TMC4671_HALL_PHI_E_SHIFT);
		...
		if(*hall_phi_e_old != *hall_phi_e_new)
		{
			...
			int16_t abn_phi_e_actual = (int16_t) tmc4671_readRegister16BitValue(TMC4671_ABN_DECODER_PHI_E_PHI_M, BIT_16_TO_31) - *hall_actual_coarse_offset;
...
uint16_t tmc4671_getAdcI0Offset()
{
...
uint16_t tmc4671_getAdcI1Offset()
{
