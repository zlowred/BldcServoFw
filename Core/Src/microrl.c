...
inline static void print_prompt(microrl_t *pThis) {
	if (pThis->get_prompt == NULL) {
		pThis->print(pThis->prompt_str);
	} else {
		pThis->print("\033[32m");
		pThis->print(pThis->get_prompt());
		pThis->print(" > \033[0m");
	}
}
...
static void terminal_reset_cursor(microrl_t *pThis) {
	char str[20];
	int promptLength =
			pThis->get_prompt == NULL ?
			_PROMPT_LEN :
										strlen(pThis->get_prompt()) + 3;
#ifdef _USE_LIBC_STDIO
	snprintf(str, 20, "\033[%dD\033[%dC",
	_COMMAND_LINE_LEN + promptLength + 2, promptLength);

#else
	char *endstr;
	strcpy (str, "\033[");
	endstr = u16bit_to_str ( _COMMAND_LINE_LEN + promptLength + 2,str+2);
	strcpy (endstr, "D\033["); endstr += 3;
	endstr = u16bit_to_str (promptLength, endstr);
	strcpy (endstr, "C");
#endif
	pThis->print(str);
}
...
//*****************************************************************************
void microrl_set_prompt_callback(microrl_t *pThis, char* (*get_prompt)()) {
	pThis->get_prompt = get_prompt;
}
	...
	if (compl_token[0] != NULL) {
		...
		if (compl_token[1] == NULL) {
			...
			while (compl_token[i] != NULL) {
				if (i == 0 || compl_token[0] != compl_token[i]) {
